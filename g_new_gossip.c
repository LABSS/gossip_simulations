...
//RETE SW (WATTS-STROGATZ) PER FASE GOSSIP E RIPRODUZIONE
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
#include<time.h>
#include"ran2.h"
#define M 50                  //numero di gruppi
#define G 8                   //taglia dei gruppi
#define VG0 2                  //numero vicini in avanti nella rete SW
#define F 0.5                 //frazione di membri del gruppo fissi. Il resto assegnati random
...
#define P_ERR 0.3             //probabilità errore di trasmissione info (cattivo al posto di buono)

#define pr 0.05               //probabilità di rewiring

#define PHASES 3              //numero di fasi gioco+gossip dentro una generazione
#define T_MAX 1000             //tempo massimo
#define S 2000                //numero di realizzazioni su cui mediare

main(){

  FILE *fa,*fb,*fc;
  int i,i2,j,j0,k,N,GG,r,t,tau,p,h,qq,tries,counter,sort0,sort,vbl,iter,kappa,jkl,new,VG;
  unsigned long int tt;
  int m[G],comp[M][G];
  double B,extr,pot,s_m,sort1,qwe,extrW;
  ...
  for (r=0;r<S;r++){
    ...
    /////////////COSTRUZIONE DELLA RETE SW

    ///////QUI COSTRUZIONE MATRICE DI ADIACENZA
    for (i=0;i<N-1;i++){
      for (j=i+1;j<N;j++){
	kappa=j-i;
	if (kappa>N/2)
	...
	minollo:
	  new=N*ran2(a);
	  if (new==i || new==jkl || matrix[i][new]==1)
	    goto minollo;
	  else {
	    matrix[i][new]=1;
	    matrix[new][i]=1;
    ...
    for (i=0;i<N;i++){
      printf("nv[%d]=%d\n",i,nv[i]);
      total_nn=total_nn+nv[i];
    }
    printf("\ntot_links=%d\n",total_nn/2);//FINE CONTROLLI
    */
    //////FINE COSTRUZIONE MATRICE DI ADIACENZA

    ///////////FINE COSTRUZIONE RETE SW
	...
	if (str[i2]==1)
	  coop[t+1]++;
                             ...
                             //plottigat
    if ((fa=fopen("don_freq_M050G08F0.50B0.85Perr0.30_pr0.05.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq_M050G08F0.50B0.85Perr0.30_pr0.05.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image_M050G08F0.50B0.85Perr0.30_pr0.05.dat","w"))==NULL){
