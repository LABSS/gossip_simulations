//MODELLO GOSSIP ISPIRATO A ESPERIMENTO VALENCIA - 
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
...
#define S 5000                //numero di realizzazioni su cui mediare
...
main(){
  ...
  for (r=0;r<S;r++){
	...
	paperbat:
	  ...
	  if (sort==i || sort==j)
	    goto paperbat;
	  //printf ("\n%g\t%g\t",s[i][sort],s[j][sort]);
	  s[j][sort]=(2.0*s[j][sort]+1.0*s[i][sort])/3.0;
	  //printf ("\t%g\n",s[j][sort]);
	...
	if (str[i2]==1)
	  coop[t+1]++;
                    ...
                    //plottigat
    if ((fa=fopen("don_freq.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image.dat","w"))==NULL){
