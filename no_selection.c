//MODELLO GOSSIP ISPIRATO A ESPERIMENTO VALENCIA - NO PARTNER SELECTION
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
...
main(){
  ...
  FILE *fa,*fb,*fc;
  int i,i2,j,k,N,GG,r,tau,t,p,h,qq,tries,counter,sort0,sort,vbl,iter;
  ...
  for (r=0;r<S;r++){
	...
	if (str[i2]==1)
	  coop[t+1]++;
                        ...
                        //plottigat
    if ((fa=fopen("don_freq_nosel.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq_nosel.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image_nosel.dat","w"))==NULL){
