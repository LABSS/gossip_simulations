...
//ERRORI UNIDIREZIONALI AL CONTRARIO(cattivi segnalati come buoni, mai viceversa), UGUALE PESO DI ESPERIENZA DIRETTA E GOSSIP
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
...
#define M 67                  //numero di gruppi
#define G 6                   //taglia dei gruppi
...
#define P_ERR 0.15             //probabilità errore di trasmissione info (buono al posto di cattivo)
  ...
  for (r=0;r<S;r++){
	...
	if (str[i2]==1)
	  coop[t+1]++;
                                           ...
                                           //plottigat
    if ((fa=fopen("don_freq_M067G06F0.50B0.85_phases05Perr0.15.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq_M067G06F0.50B0.85_phases05Perr0.15.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image_M067G06F0.50B0.85_phases05Perr0.15.dat","w"))==NULL){
