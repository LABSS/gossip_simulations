//MODELLO GOSSIP ISPIRATO A ESPERIMENTO VALENCIA - FASE GOSSIP SILENZIATA!!!
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
...
#define T_MAX 200             //tempo massimo
#define S 10000                //numero di realizzazioni su cui mediare
  ...
  for (r=0;r<S;r++){
	...
	if (str[i2]==1)
	  coop[t+1]++;
                             ...
                             //plottigat
    if ((fa=fopen("don_freq_M100G04F0.50B0.85NOGOSSIP.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq_M100G04F0.50B0.85NOGOSSIP.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image_M100G04F0.50B0.85NOGOSSIP.dat","w"))==NULL){
