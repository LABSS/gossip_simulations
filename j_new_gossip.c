...
//SCRITTURA DELLA DISTRIBUZIONE DI REPUTAZIONI (s[i][j]) INCLUSA
#include<stdio.h>   
#include<stddef.h>
#include<stdlib.h>
#include<math.h>
...
#define M 20                  //numero di gruppi
#define G 20                   //taglia dei gruppi
...
#define S 50//00                //numero di realizzazioni su cui mediare
...
int main(){
  ...
  a2=-2573450310;//-(unsigned)time(&Time);//-630598617;//
  a=&a2;
  ...
  for (r=0;r<S;r++){
	...
	if (i!=j)
	  printf("%g\n",s[i][j]);
                                      ...
                                      //plottigat
    if ((fa=fopen("don_freq_M020G20F0.50B0.85_phases05Perr0.00.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("coopfreq_M020G20F0.50B0.85_phases05Perr0.00.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("av_image_M020G20F0.50B0.85_phases05Perr0.00.dat","w"))==NULL){
      printf ("Impossibile aprire il file-c \n");
      exit (1);
    }
    /*if ((fd=fopen("im_distr_M020G20F0.50B0.85_phases05Perr0.00.dat","w"))==NULL){
